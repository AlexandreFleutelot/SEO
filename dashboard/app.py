...
def main():
    ...
    try:
        ...
        with st.sidebar:
            ...
            st.write(f"**URL:** {selected_report['url']}")
            st.write(f"**Domaine:** {selected_report['domain']}")
            st.write(f"**Date:** {selected_report['created_date'].strftime('%d/%m/%Y à %H:%M')}")
            st.write(f"**Scores disponibles:** {'✅' if selected_report['has_scores'] else '❌'}")
        ...
        # Chargement des données du rapport sélectionné
        ...
        # Section comparaison si plusieurs rapports disponibles
        if len(reports) > 1:
            st.divider()
            display_comparison_section(loader, reports)
...
def display_comparison_section(loader, reports):
    """Affiche la section de comparaison entre rapports."""
    st.header("📊 Comparaison de Rapports")
    
    # Sélection multiple de rapports
    selected_reports = st.multiselect(
        "Sélectionner les rapports à comparer",
        options=reports,
        default=reports[:min(3, len(reports))],  # Sélectionner les 3 premiers par défaut
